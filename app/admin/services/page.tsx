...
export default function AdminServices() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = formData.id ? `/api/admin/services/${formData.id}` : "/api/admin/services";
    ...
    try {
      ...
      if (res.ok) {
        setIsEditing(false);
        setFormData({ name: "", slug: "", description: "", price: "", priceUnit: "per_session", isActive: true });
        fetchItems();
      }
    } catch (err) {
  ...
  return (
    <div>
      <div className="title-row">
        <h1>Services</h1>
        <button onClick={() => { setIsEditing(true); setFormData({ name: "", slug: "", description: "", price: "", priceUnit: "per_session", isActive: true }); }} className="btn btn-primary">
          Add Service
        </button>
      </div>
      ...
      {isEditing && (
        <div className="card">
          <h2>{formData.id ? "Edit" : "Add"} Service</h2>
          <form onSubmit={handleSubmit}>
            ...
            {formData.id && (
              <MediaManager entityType="service" entityId={formData.id} />
            )}
